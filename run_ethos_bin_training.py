...
def train(model, tokenizer, ds_train, ds_eval):
    ...
    scheduler = get_cosine_schedule_with_warmup(
        optim, num_warmup_steps=len(ds_train) * 2, num_training_steps=len(ds_train) * args.num_train_epochs)
...
def main():
    ...
    ds_train = EthosBinaryDataset(ds_train)
    ds_eval = EthosBinaryDataset(ds_eval)
